...
try:
    ...
    GUI_AVAILABLE = 'DISPLAY' in os.environ
except ImportError:
    GUI_AVAILABLE = False
...
class ChaoticImageGenerator:
    def __init__(self, width=256, height=256, map_type='logistic'):
        if map_type not in ['logistic', 'tent', 'henon']:
    ...
    def generate_sequence(self, x0, param, iterations):
        # Secure random seed adjustment
        x0 = (x0 + int.from_bytes(os.urandom(4), 'big') / 2**32) % 1.0
    ...
    def derive_key(self, sequence):
        ...
        kdf = PBKDF2HMAC(
            ...
            length=32,
            salt=salt,
            iterations=500000,  # Increased iterations for key stretching
        )
    ...
    def generate_image(self, x0, param):
        ...
        if self.chaotic_map.map_type == 'logistic' and not 0 <= param <= 4:
            raise ValueError("Logistic map parameter must be between 0 and 4")
        if self.chaotic_map.map_type == 'tent' and not 0 <= param <= 2:
            raise ValueError("Tent map parameter must be between 0 and 2")
        if self.chaotic_map.map_type == 'henon' and not 0 <= param <= 1.4:
            raise ValueError("Henon map parameter must be between 0 and 1.4")
...
def run_cli():
    print("Chaotic Image Generator (CLI Mode)")
    try:
        x0 = float(input("Enter initial condition (x0, e.g., 0.5): ") or 0.5)
        if not 0 <= x0 <= 1:
            raise ValueError("x0 must be between 0 and 1")
        param = float(input("Enter parameter (r, mu, or a, e.g., 3.9): ") or 3.9)
        map_type = input("Enter map type (logistic, tent, henon): ") or "logistic"
        if map_type not in ['logistic', 'tent', 'henon']:
            raise ValueError("Invalid map type")
        filename = input("Enter output filename (e.g., output.png): ") or "output.png"
        if not re.match(r'^[\w\-.]+\.png$', filename):
            raise ValueError("Invalid filename")

        generator = ChaoticImageGenerator(map_type=map_type)
        image_data = generator.generate_image(x0, param)
